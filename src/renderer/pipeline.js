...
export class PaintPipeline {
    ...
    pipeline;
    bindGroup;
    resolutionBuffer; // 1. Declared as a property
    constructor(device, context, format, canvasWidth, canvasHeight) {
        ...
        this.device.queue.writeBuffer(this.resolutionBuffer, // Use "this." here
        0, new Float32Array([canvasWidth, canvasHeight, 0.05, 0]));
    ...
    draw(stamps) {
        ...
        encoder.copyTextureToTexture({ texture: this.renderTarget }, { texture: canvasTexture }, [canvasTexture.width, canvasTexture.height, 1]);
        this.device.queue.submit([encoder.finish()]);
        // Cleanup
        instanceBuffer.destroy();
    }
    resize(newWidth, newHeight, currentBrushSize) {
        ...
        encoder.copyTextureToTexture({ texture: this.renderTarget }, { texture: newRenderTarget }, [copyWidth, copyHeight, 1]);
        // 4. Immediately show this to the screen
        const canvasTexture = this.context.getCurrentTexture();
        encoder.copyTextureToTexture({ texture: newRenderTarget }, { texture: canvasTexture }, [Math.min(newWidth, canvasTexture.width), Math.min(newHeight, canvasTexture.height), 1]);
        this.device.queue.submit([encoder.finish()]);
        // Clean up
    ...
    clear() {
        ...
        const canvasTexture = this.context.getCurrentTexture();
        encoder.copyTextureToTexture({ texture: this.renderTarget }, { texture: canvasTexture }, [this.renderTarget.width, this.renderTarget.height, 1]);
        this.device.queue.submit([encoder.finish()]);
    }
    async saveImage() {
